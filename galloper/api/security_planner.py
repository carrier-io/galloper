...
class TestsApiSecurityDAST(Resource):
    ...
    _post_rules = (
        ...
        dict(name="dast_target_url", type=str, location='form'),
    )
...
class TestApiSecurityDAST(Resource):
    ...
    _put_rules = (
        dict(name="dast_target_url", type=str, required=False, location='json'),
    )
    ...
    def put(self, project_id, test_id):
        ...
        #
        if args.get("dast_target_url"):
            task.dast_target_url = args["dast_target_url"]
        #
        task.commit()
        return task.to_json()
