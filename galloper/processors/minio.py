...
def create_bucket(name):
    try:
        if name not in list_bucket():
            return get_client().create_bucket(ACL='public-read', Bucket=name, 
...
def list_files(bucket_name):
    ...
    files = [{'name': each['Key'], 'size': each['Size'], 'modified': each['LastModified']} for each in response.get('Contents', {})]
    continuation_token = response.get('NextContinuationToken')
    while continuation_token and response['Contents']:
        response = get_client().list_objects_v2(Bucket=bucket_name, ContinuationToken=continuation_token)
        appendage = [{'name': each['Key'], 'size': each['Size'], 'modified': each['LastModified']} for each in response.get('Contents', {})]
        if not appendage:
            break
        files += appendage
        continuation_token = response.get('NextContinuationToken')
