...
class APIDataManager:
    ...
    def prepare_api_test_data(self):
        ...
        if baseline:
            ...
            previous_test = list(tests_data[1].get_points())
        else:
            previous_test = None
    ...
    def create_api_test_summary(self, test, baseline, comparison_metric):
        ...
        test_params['status'], test_params['color'], test_params['failed_reason'] = self.check_api_test_status(test,
                                                                                        baseline, comparison_metric)
        return test_params

    def check_api_test_status(self, test, baseline, comparison_metric):
        failed_reasons = []
        test_status, failed_message = self.check_api_test_functional_issues(test)
        if failed_message != '':
            failed_reasons.append(failed_message)
        status, failed_message = self.check_api_test_performance_degradation(test, baseline, comparison_metric)
        if failed_message != '':
            failed_reasons.append(failed_message)
        if test_status is 'SUCCESS':
            test_status = status
        status, failed_message = self.check_api_test_missed_thresholds(test, comparison_metric)
        if failed_message != '':
            failed_reasons.append(failed_message)
        if test_status is 'SUCCESS':
            test_status = status
